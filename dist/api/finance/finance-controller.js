...
exports.getTotalOrderedProduct = exports.getTotalSumRevenue = exports.getProductStatSum = void 0;
...
const getTotalOrderedProduct = (req, res) => {
    db_1.default.query('SELECT SUM (quantity) as total_order from order_product INNER JOIN product on order_product.product_id = product.product_id INNER JOIN orders on order_product.order_id = orders.order_id where done = true', (err, result) => {
        if (err) {
            throw err;
        }
        res.status(200).json({ data: result.rows });
...
exports.getTotalOrderedProduct = getTotalOrderedProduct;
